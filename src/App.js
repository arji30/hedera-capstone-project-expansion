...
function App() {
  ...
  const createCar = async (cid, mintPrice) => {
    //adding mintPrice
    try {
  ...
  const borrowCar = async (tokenSolidityAddress, serial) => {
    ...
    try {
      if (!contract) getContract();
      const result = await contract.mintingPrices(serial);
      ...
      const tx = await contract.borrowing(tokenSolidityAddress, serial, {
        value: ethers.utils.parseEther(decimalValue.toString()),
        gasLimit: 2_000_000,
      });
      await tx.wait();
    ...
    <>
      ...
      <Routes>
        {isMerchant ? (
          ...
          <>
            <Route path="/" element={<Borrow borrowCar={borrowCar} contract={contract || getContract()} />} />
            <Route
              path="/give"
              element={
