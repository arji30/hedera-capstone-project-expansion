...
function Borrow({ borrowCar, contract }) {
  ...
  useEffect(() => {
    ...
    //fetch nft price using the serial number
    const fetchDecimalValues = async () => {
        const values = {};
        for (const nft of data?.nfts || []) {
          try {
            const result = await contract.mintingPrices(nft.serial_number);
            const decimalValue = parseInt(result._hex, 16);
            values[nft.serial_number] = decimalValue;
          } catch (error) {
            console.log(`Error fetching decimal value for serial number ${nft.serial_number}:`, error);
          }
        }
        setDecimalValues(values);
    };

    readData();
    fetchDecimalValues();
  }, [contractAddress, flag, contract, data]);
            ...
            <div className="item">
              <table>
                <tbody>
                  ...
                  <tr>
                    <td className="title">Priced at: </td>
                    {/* set the value to loading if value is not loaded */}
                    <td className="desc">{decimalValues[nft.serial_number]||"...loading"}</td> 
                  </tr>
                </tbody>
              </table>
